...
export default function Integrations(){
 const [tg,setTg]=useState<Tg|null>(null);const [secret,setSecret]=useState("");const [msg,setMsg]=useState("");
 ...
 useEffect(()=>{load()},[]);
 async function setup(){setMsg("Mengaktifkan webhook...");const r=await fetch("/api/telegram/setup",{method:"POST",headers:{"x-telegram-setup-secret":secret}});const d=await r.json();setMsg(d.ok?"Webhook Telegram aktif. Kirim /start ke bot.":d.error||"Gagal");if(d.ok){setSecret("");await load()}}
 ...
 <div className="card sectionless">
  ...
  <div className="telegramDetail">
   {!tg?.configured&&<div className="notice">Tambahkan <b>TELEGRAM_BOT_TOKEN</b> dan <b>TELEGRAM_WEBHOOK_SECRET</b> di Vercel Environment Variables. Token tidak disimpan di web atau GitHub.</div>}
   ...
   {tg?.configured&&!tg.webhook?.url&&<div className="setupRow"><input className="input" type="password" placeholder="Masukkan TELEGRAM_WEBHOOK_SECRET untuk setup sekali" value={secret} onChange={e=>setSecret(e.target.value)}/><button className="btn primary" disabled={!secret} onClick={setup}>Aktifkan Webhook</button></div>}
 ...
 <div className="notice section"><b>Alur Telegram:</b> Telegram → Vercel webhook → My Control. Untuk sekarang /start dan /status sudah bisa aktif. /rekap, /pending, /closing, dan /payment akan membaca data yang sama dengan web setelah Supabase tersambung.</div></>
}